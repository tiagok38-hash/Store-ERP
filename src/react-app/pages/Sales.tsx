...
export default function Sales() {
  ...
  const [isRankingCollapsed, setIsRankingCollapsed] = useState(true); // Novo estado para o ranking
    ...
    <div className={`min-h-screen transition-colors duration-200 ${
      ...
      <div className="p-6 pt-0">
        <div className={`rounded-lg shadow-lg overflow-hidden mb-6 ${
          ...
          <div className={`p-4 border-b ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'} flex justify-between items-center`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={isRankingCollapsed ? 'Expandir ranking' : 'Recolher ranking'}
              >
                {isRankingCollapsed ? (
                  <ChevronDown size={20} className={theme === 'dark' ? 'text-slate-300' : 'text-slate-500'} />
                ) : (
                  <ChevronUp size={20} className={theme === 'dark' ? 'text-slate-300' : 'text-slate-500'} />
                )}
              </button>
            </div>
          </div>
          ...
          {!isRankingCollapsed && (
            <div className="overflow-x-auto animate-page-fade-in"> {/* Adicionado animação */}
