...
export default function Inventory() {
  ...
  const [purchaseToEdit, setPurchaseToEdit] = useState<Purchase | undefined>(undefined); // State for editing an existing purchase
  const [purchaseToView, setPurchaseToView] = useState<Purchase | undefined>(undefined); // State for viewing purchase
  const [productToAddToNewPurchase, setProductToAddToNewPurchase] = useState<Product | undefined>(undefined); // NEW: State for pre-filling PurchaseModal
  ...
  // MODIFIED: handleEditProduct now opens PurchaseModal with the product pre-filled
  const handleEditProduct = (product: Product) => {
    setProductToAddToNewPurchase(product); // Set the product to pre-fill the purchase modal
    setIsPurchaseModalOpen(true); // Open the PurchaseModal
    // Note: The original ProductModal for editing product definitions is still accessible via "Novo Produto" -> "Editar" if needed,
    // or a separate button could be added for direct product definition editing.
  };
  ...
  const handlePurchaseCreated = async (newPurchase: Purchase) => {
    ...
    setProductToAddToNewPurchase(undefined); // Clear this state after purchase is created/attempted
  };
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
                    ...
                    {products.length === 0 ? (
                      ...
                      products.map((product) => (
                        <tr key={product.id} className={`${theme === 'dark' ? 'hover:bg-slate-700' : 'hover:bg-slate-50'}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button
                              onClick={() => handleEditProduct(product)} // Now opens PurchaseModal
      ...
      <PurchaseModal
        ...
        onClose={() => {
          setIsPurchaseModalOpen(false);
          setProductToAddToNewPurchase(undefined); // Clear the pre-filled product state
          setPurchaseToEdit(undefined); // Clear the editing purchase state
        }}
        ...
        initialProductToAdd={productToAddToNewPurchase} // NEW: Pass the product to pre-fill
      />
      {purchaseToFinalize && (
