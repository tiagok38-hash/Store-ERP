    ...
    <div className="p-6 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-600">Configure os métodos de pagamento aceitos</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg p-4 shadow-md">
          <h3 className="text-sm font-semibold text-slate-800 mb-2">Total de Métodos</h3>
          <p className="text-xl font-bold text-blue-500">{paymentMethods.length}</p>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-md">
          <h3 className="text-sm font-semibold text-slate-800 mb-2">Métodos Ativos</h3>
          <p className="text-xl font-bold text-green-500">{paymentMethods.filter(m => m.isActive).length}</p>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-md">
          <h3 className="text-sm font-semibold text-slate-800 mb-2">Com Taxa</h3>
          <p className="text-xl font-bold text-orange-500">{paymentMethods.filter(m => m.feePercentage > 0).length}</p>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-md">
          <h3 className="text-sm font-semibold text-slate-800 mb-2">Cartões de Crédito</h3>
                ...
                <div className="flex items-center flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-slate-800">{method.name}</h3>
                    ...
                    {method.type === 'credit' && method.creditOptions && (
                      <div className="mt-3 p-3 bg-slate-50 rounded-lg">
                        <h4 className="text-sm font-medium text-slate-700 mb-2">Configurações do Cartão:</h4>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                          {Object.entries(method.creditOptions.interestRates).map(([installments, rate]) => (
                            <div key={installments} className="bg-white p-2 rounded border">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center pt-6">
                    <label className="flex items-center">
                      ...
                      />
                      <span className="ml-2 text-sm text-slate-700">Requer aprovação</span>
                    </label>
                  </div>
                </div>
                ...
                {formData.type === 'credit' && (
                  <div className="border-t pt-6">
                    <h3 className="text-lg font-semibold text-slate-800 mb-4">Configurações do Cartão de Crédito</h3>
                      ...
                      <div className="flex items-center gap-4">
                        <label className="flex items-center">
                          <input
                            ...
                            className="rounded border-slate-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                          />
                          <span className="ml-2 text-sm text-slate-700">Permitir parcelamento sem juros</span>
                        </label>

                        {formData.allowInterestFree && (
                          <div className="flex items-center gap-2">
                            <label className="text-sm text-slate-700">Até:</label>
                            <input
                              type="number"
                              min="1"
                              max="12"
                            ...
                            />
                            <span className="text-sm text-slate-700">parcelas</span>
                          </div>
                        )}
                      </div>

                      <div>
                        <h4 className="text-sm font-medium text-slate-700 mb-3">Taxa de Juros por Parcela:</h4>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {Array.from({ length: 16 }, (_, i) => i + 3).map(installments => (
                            <div key={installments} className="flex items-center gap-2">
                              <label className="text-sm text-slate-700 w-8">{installments}x:</label>
                              <input
                                type="number"
                                step="0.1"
                                min="0"
                                ...
                                className="flex-1 px-2 py-1 border border-slate-300 rounded text-sm focus:ring-2 focus:ring-blue-500"
                                placeholder="0.0"
                              />
